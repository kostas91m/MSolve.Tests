...
namespace MGroup.FEM.Structural.Tests.Integration
{
	...
	public class TwoTrussesContactTest
	{
		...
		public void RunTest()
		{
			var model = TwoTrussesContact.CreateModel();
			var log = SolveModel(model);
			Assert.Equal(expected: TwoTrussesContact.expected_solution_node3_TranslationX, actual: log.DOFValues[watchDofs[0].node, watchDofs[0].dof], precision: 3);
		}
